...
// ── Graduation Speed Components ──────────────────────────────────────────────
function GradSpeedBadge({ bucket }) {
  if (bucket === 'fast')   return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700">⚡ Faster</span>;
  if (bucket === 'slow')   return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-600">🐢 Slower</span>;
  if (bucket === 'onTime') return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-100 text-blue-600">🎯 On Time</span>;
  return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-600">⏳ In Progress</span>;
}

function YearsDistributionChart({ data }) {
  if (!data || data.length === 0) {
    return (
      <div className="h-64 flex items-center justify-center bg-gray-50 rounded-lg">
        <p className="text-sm text-gray-400">No graduation data available</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      <h3 className="text-sm font-semibold text-gray-700 mb-4">Years-to-Graduation Distribution</h3>
      <div className="relative" style={{ height: `${height}px` }}>
        <svg width="100%" height={height} viewBox={`0 0 ${width + padding.left + padding.right} ${height}`} preserveAspectRatio="none">
          <text x="12" y={height / 2} className="text-[10px] fill-gray-400" textAnchor="middle" transform={`rotate(-90, 12, ${height / 2})`}>
            Students
          ...
          {[0, 0.25, 0.5, 0.75, 1].map(p => {
            ...
            return (
              <g key={p}>
                <line x1={padding.left - 10} y1={y} x2={padding.left - 5} y2={y} stroke="#cbd5e1" strokeWidth="1" />
                <text x={padding.left - 15} y={y + 3} className="text-[9px] fill-gray-400" textAnchor="end">{val}</text>
                <line x1={padding.left} y1={y} x2={width + padding.left} y2={y} stroke="#e2e8f0" strokeWidth="0.5" strokeDasharray="4" />
              </g>
            );
          })}
...
function GraduationSpeedView({ data }) {
  if (!data) {
    return <div className="bg-white rounded-2xl border border-gray-200 p-6 text-sm text-gray-400">No graduation speed data available.</div>;
  }

  const { summary, yearsDistribution, courseSpeedStats, slowestStudents, fastestStudents } = data;

  return (
    <div className="space-y-6">
      {/* Summary cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <StatCard label="Classified" value={summary.classified} sub="graduated students" icon="🎓" color="blue" />
        <StatCard label="In Progress" value={summary.inProgress} sub="still studying" icon="⏳" color="slate" />
        <StatCard label="On Time" value={`${summary.onTimePercent}%`} sub={`${summary.onTime} students`} icon="🎯" color="blue" />
        <StatCard label="Faster" value={`${summary.fastPercent}%`} sub={`${summary.fast} students`} icon="⚡" color="green" />
        <StatCard label="Slower" value={`${summary.slowPercent}%`} sub={`${summary.slow} students`} icon="🐢" color="red" />
        <StatCard
          label="Avg Delta"
          value={`${summary.avgDelta >= 0 ? '+' : ''}${summary.avgDelta} yr`}
          sub={`exp ${summary.avgExpectedYears}y vs act ${summary.avgActualYears}y`}
          icon="📐"
          color={summary.avgDelta >= 0.25 ? 'red' : summary.avgDelta <= -0.25 ? 'green' : 'amber'}
        />
      </div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl border border-indigo-100 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-800">Graduation Pace Insight</h3>
            <p className="text-xs text-gray-600 mt-1">
              Across {summary.classified} graduated student(s), the average time to degree is{' '}
              <strong>{summary.avgActualYears} years</strong> vs an expected{' '}
              <strong>{summary.avgExpectedYears} years</strong>
              {summary.avgDelta > 0 ? ` — graduating ${summary.avgDelta} year(s) slower than planned on average.` : ''}
              {summary.avgDelta < 0 ? ` — graduating ${Math.abs(summary.avgDelta)} year(s) faster than planned on average.` : ''}
              {summary.avgDelta === 0 ? ' — right on schedule overall.' : ''}
              {summary.slowPercent > 30 && ` ${summary.slowPercent}% of students extended beyond their course duration — review unit-level failure patterns for the most-failed units (likely contributors).`}
            </p>
          </div>
        </div>
      </div>

      {/* Distribution chart + bucket breakdown */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <YearsDistributionChart data={yearsDistribution} />
          <div className="mt-3 pt-3 border-t border-gray-100 flex justify-between text-xs text-gray-400">
            <span>📊 Each bar represents students grouped into half-year bins by actual graduation time.</span>
          </div>
        </div>

        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">Pace Breakdown</h3>
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-xs text-gray-600 mb-1">
                <span>⚡ Faster than expected</span>
                <span>{summary.fast} ({summary.fastPercent}%)</span>
              </div>
              <div className="w-full h-2 bg-gray-100 rounded-full">
                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${summary.fastPercent}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-gray-600 mb-1">
                <span>🎯 On time</span>
                <span>{summary.onTime} ({summary.onTimePercent}%)</span>
              </div>
              <div className="w-full h-2 bg-gray-100 rounded-full">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${summary.onTimePercent}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-gray-600 mb-1">
                <span>🐢 Slower than expected</span>
                <span>{summary.slow} ({summary.slowPercent}%)</span>
              </div>
              <div className="w-full h-2 bg-gray-100 rounded-full">
                <div className="h-full bg-red-500 rounded-full" style={{ width: `${summary.slowPercent}%` }} />
              </div>
            </div>
            <div className="pt-3 border-t border-gray-100 text-[11px] text-gray-500 leading-relaxed">
              <p><strong>Faster</strong> = finished at least 0.25 yr early.</p>
              <p><strong>On time</strong> = within ±0.25 yr of expected.</p>
              <p><strong>Slower</strong> = finished at least 0.25 yr late.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="text-sm font-semibold text-gray-700">Average Graduation Speed by Course</h3>
          <p className="text-xs text-gray-400 mt-0.5">Expected duration vs actual average. Sorted by delay (worst at top).</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">Students</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">Expected (y)</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">Actual avg (y)</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">Delta</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">⚡ Fast</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">🎯 On time</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500">🐢 Slow</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {(courseSpeedStats || []).map(c => (
                <tr key={c.courseCode} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center text-xs text-gray-600">{c.avgExpectedYears}</td>
                  <td className="px-4 py-3 text-center text-xs text-gray-600">{c.avgActualYears}</td>
                  <td className={`px-4 py-3 text-center text-xs font-bold ${c.avgDelta >= 0.25 ? 'text-red-600' : c.avgDelta <= -0.25 ? 'text-emerald-600' : 'text-blue-600'}`}>
                    {c.avgDelta >= 0 ? '+' : ''}{c.avgDelta}y
                  </td>
                  <td className="px-4 py-3 text-center text-xs text-emerald-600">{c.fast}</td>
                  <td className="px-4 py-3 text-center text-xs text-blue-600">{c.onTime}</td>
                  <td className="px-4 py-3 text-center text-xs text-red-600">{c.slow}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {(courseSpeedStats || []).length === 0 && (
            <div className="text-center py-12 text-gray-400 text-sm">No course-level data available</div>
          )}
        </div>
      </div>

      {/* Slowest & fastest students */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 bg-red-50/30">
            <h3 className="text-sm font-semibold text-gray-700">🐢 Slowest Graduators</h3>
            <p className="text-xs text-gray-400 mt-0.5">Most delayed beyond expected duration.</p>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-4 py-2 text-center text-xs font-semibold text-gray-500">Exp</th>
                <th className="px-4 py-2 text-center text-xs font-semibold text-gray-500">Actual</th>
                <th className="px-4 py-2 text-center text-xs font-semibold text-gray-500">Delta</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {(slowestStudents || []).map(s => (
                <tr key={s.studentID} className="hover:bg-red-50/30">
                  <td className="px-4 py-2 text-xs text-gray-700">{s.name}</td>
                  <td className="px-4 py-2 text-xs text-gray-500 font-mono">{s.courseCode}</td>
                  <td className="px-4 py-2 text-center text-xs text-gray-500">{s.expectedYears}y</td>
                  <td className="px-4 py-2 text-center text-xs text-gray-700">{s.actualYears}y</td>
                  <td className="px-4 py-2 text-center text-xs font-bold text-red-600">+{s.deltaYears}y</td>
                </tr>
              ))}
            </tbody>
          </table>
          {(slowestStudents || []).length === 0 && (
            <div className="text-center py-8 text-gray-400 text-xs">No slow graduators identified</div>
          )}
        </div>

        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 bg-emerald-50/30">
            <h3 className="text-sm font-semibold text-gray-700">⚡ Fastest Graduators</h3>
            <p className="text-xs text-gray-400 mt-0.5">Finished earliest vs expected duration.</p>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100">
              {(fastestStudents || []).map(s => (
                <tr key={s.studentID} className="hover:bg-emerald-50/30">
                  <td className="px-4 py-2 text-xs text-gray-700">{s.name}</td>
                  <td className="px-4 py-2 text-xs text-gray-500 font-mono">{s.courseCode}</td>
                  <td className="px-4 py-2 text-center text-xs text-gray-500">{s.expectedYears}y</td>
                  <td className="px-4 py-2 text-center text-xs text-gray-700">{s.actualYears}y</td>
                  <td className="px-4 py-2 text-center text-xs font-bold text-emerald-600">{s.deltaYears}y</td>
                </tr>
              ))}
            </tbody>
          </table>
          {(fastestStudents || []).length === 0 && (
            <div className="text-center py-8 text-gray-400 text-xs">No accelerated graduators identified</div>
          )}
        </div>
      </div>
    </div>
...
export default function UnitAnalyticsPage() {
  ...
  const { summary, topByFailRate, topByFailCount, topByRepeats, termTrend, worstPerformingUnits, graduationSpeed } = data;
  ...
  const tabs = [
    ...
    { id: 'gradspeed', label: 'Graduation Speed', icon: '🎓' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div>
          <div className="flex flex-wrap gap-3 items-center mb-5">
            ...
            {activeTab !== 'overview' && activeTab !== 'gradspeed' && (
              <>
                <input
                  type="text"
          ...
          {/* Graduation Speed View */}
          {activeTab === 'gradspeed' && (
            <GraduationSpeedView data={graduationSpeed} />
          )}
