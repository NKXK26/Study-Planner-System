...
function RiskBadge({ rate }) {
    if (rate === null || rate === undefined) return <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-500">📭 No Data</span>;
...
function FailRateBar({ passCount, failCount, totalAttempts }) {
...
export default function UnitAnalyticsPage() {
    ...
    const filterUnits = (list) => {
        ...
        return list.filter(u => {
            ...
            // Handle units with no attempts (failRate = null)
            const failRate = u.failRate ?? 0;
            const matchRisk =
                riskFilter === 'all' ? true :
                    riskFilter === 'critical' ? failRate >= 50 :
                        riskFilter === 'high' ? failRate >= 30 && failRate < 50 :
                            riskFilter === 'medium' ? failRate >= 15 && failRate < 30 :
                                riskFilter === 'low' ? failRate < 15 && failRate > 0 :
                                    riskFilter === 'noAttempts' ? u.totalAttempts === 0 :
                                        true;
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center bg-white rounded-2xl p-8 shadow-sm">
                    ...
                    <button onClick={fetchData} className="mt-4 px-4 py-2 bg-red-500 text-white rounded-lg text-sm">Refresh</button>
                </div>
            </div>
        );
    }

    const { summary, topByFailRate, topByFailCount, topByRepeats, termTrend, worstPerformingUnits, allUnits } = data;

    // Use allUnits for complete list, fallback to other arrays
    const completeUnitList = allUnits || [...(topByFailRate || []), ...(topByFailCount || []), ...(topByRepeats || [])]
        .filter((v, i, a) => a.findIndex(t => t.unitID === v.unitID) === i);
    ...
    const tabs = [
        ...
        { id: 'all', label: `All Units (${completeUnitList.length})`, icon: '📚' },
    ];

    const getActiveList = () => {
        if (activeTab === 'failrate') return topByFailRate || [];
        if (activeTab === 'repeats') return topByRepeats || [];
        if (activeTab === 'worst') return worstPerformingUnits || [];
        if (activeTab === 'all') return completeUnitList;
        return [];
    };

    const activeList = getActiveList();

    const criticalUnits = completeUnitList?.filter(u => (u.failRate ?? 0) >= 50).length || 0;
    const highRiskUnits = completeUnitList?.filter(u => (u.failRate ?? 0) >= 30 && (u.failRate ?? 0) < 50).length || 0;
    const mediumRiskUnits = completeUnitList?.filter(u => (u.failRate ?? 0) >= 15 && (u.failRate ?? 0) < 30).length || 0;
    const healthyUnits = completeUnitList?.filter(u => (u.failRate ?? 0) < 15 && (u.failRate ?? 0) > 0).length || 0;
    const noAttemptUnits = completeUnitList?.filter(u => u.totalAttempts === 0).length || 0;
    ...
    return (
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">Complete unit analysis including units with no attempt data</p>
                        </div>
                    </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
                ...
                {/* Summary Cards - Updated to show complete picture */}
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                    <StatCard label="Total Units" value={completeUnitList.length} icon="📚" color="blue" />
                    ...
                    <StatCard label="No Attempts" value={noAttemptUnits} sub="awaiting student data" icon="📭" color="purple" />
                </div>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-600 mt-1">
                                ...
                                {noAttemptUnits > 0 && ` ${noAttemptUnits} unit(s) have no student attempt data yet.`}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                        ...
                        {activeTab !== 'overview' && (
                            <>
                                ...
                                >
                                    ...
                                    <option value="noAttempts">No Attempts Yet</option>
                                </select>
                            </>
                        )}
                    </div>

                    {/* Overview Tab - Enhanced Risk Distribution */}
                    {activeTab === 'overview' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="bg-white rounded-2xl border border-gray-200 p-6">
                                <h3 className="text-sm font-semibold text-gray-700 mb-4">Risk Distribution ({completeUnitList.length} Total Units)</h3>
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <div className="w-full h-2 bg-gray-100 rounded-full">
                                            <div className="h-full bg-red-500 rounded-full" style={{ width: `${(criticalUnits / completeUnitList.length) * 100}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-xs text-gray-600 mb-1">
                                        ...
                                        <div className="w-full h-2 bg-gray-100 rounded-full">
                                            <div className="h-full bg-amber-500 rounded-full" style={{ width: `${(highRiskUnits / completeUnitList.length) * 100}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-xs text-gray-600 mb-1">
                                            <span>Medium Risk (15-29%)</span>
                                            <span>{mediumRiskUnits} units</span>
                                        </div>
                                        <div className="w-full h-2 bg-gray-100 rounded-full">
                                            <div className="h-full bg-yellow-500 rounded-full" style={{ width: `${(mediumRiskUnits / completeUnitList.length) * 100}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-xs text-gray-600 mb-1">
                                            <span>Healthy (&lt;15%)</span>
                                            <span>{healthyUnits} units</span>
                                        </div>
                                        <div className="w-full h-2 bg-gray-100 rounded-full">
                                            <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${(healthyUnits / completeUnitList.length) * 100}%` }} />
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-xs text-gray-600 mb-1">
                                            <span>No Attempts Yet</span>
                                            <span>{noAttemptUnits} units</span>
                                        </div>
                                        <div className="w-full h-2 bg-gray-100 rounded-full">
                                            <div className="h-full bg-purple-400 rounded-full" style={{ width: `${(noAttemptUnits / completeUnitList.length) * 100}%` }} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    {noAttemptUnits > 0 && (
                                        <div className="flex items-start gap-2 p-2 bg-purple-50 rounded-lg">
                                            ...
                                            <p className="text-xs text-gray-700"><strong>Missing Data:</strong> {noAttemptUnits} unit(s) have no student attempt records. Verify unit enrollment.</p>
                                        </div>
                                    )}
                    ...
                    {/* Table Views - Now showing all units */}
                    {activeTab !== 'overview' && (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {filterUnits(activeList) && filterUnits(activeList).length > 0 ? (
                                            filterUnits(activeList).map(u => (
                                                <tr
                                                    key={u.unitID || u.unitCode}
                                                ...
                                                >
                                                    <td className="px-4 py-3 font-mono text-xs font-bold text-gray-700">{u.unitCode || 'N/A'}</td>
                                                    <td className="px-4 py-3 text-gray-700 max-w-[200px] truncate text-xs">{u.unitName || 'N/A'}</td>
                                                    <td className="px-4 py-3"><RiskBadge rate={u.failRate} /></td>
                                                    <td className="px-4 py-3 w-40">
                                                        <FailRateBar
                                                            passCount={u.passCount || 0}
                                                            failCount={u.failCount || 0}
                                                            totalAttempts={u.totalAttempts || 0}
                                                        />
                                                    </td>
                                                    <td className="px-4 py-3 text-center">
                                                        <span className={`text-sm font-bold ${(u.failRate || 0) >= 50 ? 'text-red-600' : (u.failRate || 0) >= 30 ? 'text-amber-600' : (u.failRate || 0) >= 15 ? 'text-yellow-600' : (u.failRate || 0) > 0 ? 'text-emerald-600' : 'text-gray-400'}`}>
                                                            {u.failRate !== undefined && u.failRate !== null ? `${u.failRate}%` : '—'}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3 text-center text-xs text-gray-500">{u.totalAttempts || 0}</td>
                                                    <td className="px-4 py-3 text-center text-xs text-gray-500">{u.uniqueStudents || 0}</td>
                                                    <td className="px-4 py-3 text-center">
                                                        {(u.repeatCount || 0) > 0 ? (
                ...
                <div className="text-center text-xs text-gray-400 pt-4 border-t border-gray-200">
                    <p>Data reflects all recorded unit attempts. Units with no attempts show 0 attempts and no risk assessment.</p>
