  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (top5Planners.length === 0) {
        ...
        // Count completed units by category using the top planner (first match)
        const topPlanner = top5Planners[0];
        if (topPlanner && topPlanner.totalUnits) {
          const plannerUnitMap = new Map();
          topPlanner.totalUnits.forEach(unit => {
            plannerUnitMap.set(unit.ID, unit);
          });
          let core = 0, elective = 0, major = 0;
          for (const completed of completedUnitsList) {
            const plannerUnit = plannerUnitMap.get(completed.id);
            if (plannerUnit) {
              const cat = getUnitCategory(plannerUnit);
              if (cat === 'core') core++;
