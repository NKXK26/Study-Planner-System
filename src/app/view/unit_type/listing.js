...
const UnitTypeListing = ({ params, error, HandleOpenForm, refreshList, unitTypes, setPagination }) => {
    ...
    const sortedUnitTypeListing = [...unitTypeListing].sort((a, b) => {
        // Use name field (supports both lowercase and uppercase)
        const nameA = a?.name ?? a?.Name ?? '';
        const nameB = b?.name ?? b?.Name ?? '';
        const aIsRestricted = isRestrictedUnitType(nameA);
        const bIsRestricted = isRestrictedUnitType(nameB);
    ...
    return (
        <>
            {isLoading ? (
                <tr><td colSpan="4" className="text-center py-4">Loading unit types...</td></tr>
            ) : sortedUnitTypeListing.length === 0 ? (
                <tr><td colSpan="4" className="py-8 text-gray-500 text-center">{isInitialLoad ? 'No Unit Types Found' : 'No unit types match your filters'}</td></tr>
            ) : (
                sortedUnitTypeListing
                    ...
                    .map((unitType, index) => {
                        ...
                        // Alternative colors array (lowercase)
                        const colors = unitType?.colors || [];
                        const primaryColor = unitType?.colour || '#000000';
                        return (
                            <tr
                            ...
                            >
                                ...
                                <td className="unit-type-table-cell-name">
                                    {unitType.name}
                                    {isRestricted && <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">System</span>}
                                </td>
                                <td className="unit-type-table-cell-color">
                                    <div className="flex flex-col gap-1">
                                        {/* Primary color swatch */}
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 rounded-full border border-gray-400" style={{ backgroundColor: primaryColor }}></div>
                                            <span className="font-mono">{primaryColor}</span>
                                        </div>
                                        {/* Alternative colors (from UnitTypeColor table) */}
                                        {colors.length > 0 && (
                                ...
                                <td className="unit-type-table-cell-actions">
                                    <div className="flex space-x-2">
                                        {can('unit_type', 'read') && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); HandleOpenForm('VIEW', unitType.id, unitType); }}
                                        ...
                                        {can('unit_type', 'delete') && !isRestricted && (
                                            <ActionButton
                                                actionType="delete"
                                                onClick={(e) => { e.stopPropagation(); handleDelete(unitType.id, unitType.name); }}
